@switch($type)
    @case(0)
        ...
        <p>Tên gói thầu: <i style="color: green">{{ $contract['name'] }}</i></p>
        <p>Chủ đầu tư: {{ implode(' - ', [$contract['investor']['name_vi'], $contract['investor']['name_en']]) }}</p>
        <p>Năm: {{ $contract['year'] }}</p>
        @php
            $name_chuyenmon = $contract->professionals->map(fn($i) => $i->user->name)->toArray();
            $name_giaingan = $contract->disbursements->map(fn($i) => $i->user->name)->toArray();
        @endphp
        <p>Phụ trách chuyên môn: {{ implode(', ', $name_chuyenmon) }}</p>
        <p>Phụ trách giải ngân: {{ implode(', ', $name_giaingan) }}</p>
        <p>Người yêu cầu: <b style="color: red">{{ $authUser }}</b></p>
        <p>Thời gian yêu cầu: {{ $ngay_ycpheduyet }}</p>
        <p>Đề nghị anh/chị truy cập hệ thống kiểm tra phê duyệt</p><br>
    @break
