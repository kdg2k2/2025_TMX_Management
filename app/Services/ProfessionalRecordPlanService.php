...
use App\Repositories\ProfessionalRecordPlanRepository;
use Illuminate\Support\Facades\Validator;
use Exception;

class ProfessionalRecordPlanService extends BaseService
{
    ...
    {
        $this->repository = app(ProfessionalRecordPlanRepository::class);
        ...
        $this->planDetailService = app(ProfessionalRecordPlanDetailService::class);
    }
    ...
    public function baseIndexData()
    {
        return $this->tryThrow(function () {
            return array_merge(
                app(ProfessionalRecordService::class)->baseIndexData(),
                [
        ...
        return $this->tryThrow(function () {
            {
                ...
                return asset($professionalRecordService->createExcel(
                    'uploads/professional-record/plan',
                    'professional_record_plan_' . date('d-m-Y_H-i-s') . '.xlsx',
                    $sheets
                ));
            }
        });
    ...
    public function uploadExcel(array $request)
    {
        return $this->tryThrow(function () use ($request) {
            $plan = $this->repository->findByIdContractAndYear($request['contract_id'], null);
            if (empty($plan)) {
            ...
            $data = $this->excelService->readExcel($request['file']);
            if (!isset($data['data'])) {
            ...
            $this->professionalRecordMinuteService->createPlanMinute($plan->toArray(), $request['file']);
    ...
    private function validateExcelRow(array $data, int $rowIndex = 0)
    {
        ...
        $rowData = [
            'professional_record_type' => $data[0] ?? null,
        ...
        $rules = [
            'professional_record_type' => 'required|string|exists:professional_record_types,name',
        ...
        // Custom messages
        $messages = [
            // ProfessionalRecord type messages
            'professional_record_type.required' => 'Loại biên bản là bắt buộc.',
            'professional_record_type.string' => 'Loại biên bản phải là chuỗi ký tự.',
            'professional_record_type.exists' => 'Loại biên bản ":input" không tồn tại trong hệ thống.',
            // Province messages
    ...
    private function convertNamesToIds(array $validatedRows)
    {
        ...
        $uniqueProfessionalRecordTypes = array_unique(array_filter(array_column($validatedRows, 'professional_record_type')));
        ...
        $professionalRecordTypesData = $this->professionalRecordTypeService->findByNames($uniqueProfessionalRecordTypes)->toArray();
        ...
        $getProfessionalRecordTypeId = function ($name) use ($professionalRecordTypesData) {
            foreach ($professionalRecordTypesData as $item) {
                if ($item['name'] === $name) {
                    return $item['id'];  // Dùng id từ professional_record_types
                }
            }
            return null;
        };
        ...
        // Convert each row
        return array_map(function ($row) use ($getProfessionalRecordTypeId, $getProvinceId, $getCommuneId, $getUnitId, $getUserId) {
            return [
                'professional_record_type_id' => $getProfessionalRecordTypeId($row['professional_record_type']),
    ...
    public function sendApproveRequest(int $contractId)
    {
        return $this->tryThrow(function () use ($contractId) {
            ...
            $this->professionalRecordMinuteService->sendMail(
                'Yêu cầu phê duyệt bàn giao kế hoạch chứng từ hồ sơ chuyên môn',
                [
                    'contract' => $plan->contract->load(app(ContractService::class)->repository->relations),
                    'name' => $plan->contract->name,
                    'type' => 0,
                    'ngay_ycpheduyet' => $this->formatDateTimeForPreview($lastMinute['updated_at']),
                ],
                $lastMinute
            );
        }, true);
