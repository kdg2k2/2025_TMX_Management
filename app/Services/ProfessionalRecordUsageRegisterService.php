...
use App\Models\ProfessionalRecordUsageRegister;
use App\Repositories\ProfessionalRecordUsageRegisterRepository;
use App\Services\ProfessionalRecordMinuteService;
use App\Services\ProfessionalRecordPlanService;
use Exception;

class ProfessionalRecordUsageRegisterService extends BaseService
{
    private $professionalRecordHandoverService;
    private $professionalRecordMinuteService;
    private $professionalRecordPlanService;

    public function __construct()
    {
        $this->repository = app(ProfessionalRecordUsageRegisterRepository::class);
        $this->professionalRecordHandoverService = app(ProfessionalRecordHandoverService::class);
        $this->professionalRecordMinuteService = app(ProfessionalRecordMinuteService::class);
        $this->professionalRecordPlanService = app(ProfessionalRecordPlanService::class);
    }
        ...
        return $this->tryThrow(function () use ($contractId, $year) {
            ...
            return [
                ...
                'flag' => [
                    ...
                    'minutes' => $minute ? [$this->professionalRecordMinuteService->formatRecord($minute)] : [],
                ],
            ];
        });
    }
    ...
    {
        $plan = $this->createBaseFormData($this->professionalRecordPlanService->getPlanApproved($contractId, $year));
        // Lấy tổng kho và đã sử dụng để tính hiện trạng
        $handover = $this->createBaseFormData(
            $this->professionalRecordHandoverService->getHandoverInsApproved($contractId, $year) ?? []
        );  // tổng kho
    ...
    public function formatRecord(array $array)
    {
        ...
        if (isset($array['plan']))
            $array['plan'] = $this->professionalRecordPlanService->formatRecord($array['plan']);
    ...
    public function createTempExcel(array $request)
    {
        return $this->tryThrow(function () use ($request) {
            ...
            return asset($professionalRecordService->createExcel(
                'uploads/professional-record/usage_register',
                'professional_record_usage_register_' . date('d-m-Y_H-i-s') . '.xlsx',
                $sheets
            ));
        });
    }

    public function getLastMinute(ProfessionalRecordUsageRegister $record)
    {
    ...
    public function uploadExcel(int $contractId, $file)
    {
        return $this->tryThrow(function () use ($contractId, $file) {
            ...
            $validated = array_map(function ($item) use ($usageRegister) {
                $item['professional_record_usage_register_id'] = $usageRegister['id'];
                return $item;
            }, $validated['data']);
            ...
            $this->professionalRecordMinuteService->validateMinuteStatusWhenCreate(empty($lastMinute) ? null : $lastMinute);

            $usageRegister = $this->repository->findById($usageRegister['id']);
            $this->professionalRecordMinuteService->createUsageRegisterMinute($usageRegister->toArray(), $file);
        }, true);
    }

    private function getPlanMinute(int $contractId)
    {
        $planMinute = $this->professionalRecordMinuteService->findByContractId($contractId);
        if (empty($planMinute))
    ...
    private function validateExcelData(array $originData, array $sheetData)
    {
        ...
        $professionalRecordTypeService = app(ProfessionalRecordTypeService::class);
        ...
        foreach ($originData as $index => $originRow) {
            ...
            $convertedRow = $this->convertExcelRow($sheetRow, $professionalRecordTypeService, $provinceService, $communeService, $unitService, $rowNumber, $errors);
    ...
    private function convertExcelRow($row, $professionalRecordTypeService, $provinceService, $communeService, $unitService, $rowNumber, &$errors)
    {
        $converted = [];

        // Convert cột 0: Loại biên bản → ID
        $professionalRecordType = optional($professionalRecordTypeService->findByName($row[0]))->id ?? null;
        if (!$professionalRecordType && !empty($row[0])) {
            $errors[] = "Dòng {$rowNumber}, cột 'Loại biên bản': Không tìm thấy '{$row[0]}'";
        }
        $converted['professional_record_type_id'] = $professionalRecordType;
