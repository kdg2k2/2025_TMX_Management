...
class ProfessionalRecordMinuteRepository extends BaseRepository
{
    public function __construct()
    {
        $this->model = new ProfessionalRecordMinute();
        $this->relations = [
            'plan' => function ($q) {
                $q->with(app(ProfessionalRecordPlanRepository::class)->relations);
            },
            'handover' => function ($q) {
                $q->with(app(ProfessionalRecordHandoverRepository::class)->relations);
            },
    ...
    public function deleteDraftByType(int $typeId, string $type)
    {
        return $this
            ...
            ->when($type === 'handover', function ($q) use ($typeId) {
                $q->where('professional_record_handover_id', $typeId);
            })
            ->when($type === 'plan', function ($q) use ($typeId) {
                $q->where('professional_record_plan_id', $typeId);
            })
            ->when($type === 'usage_register', function ($q) use ($typeId) {
                $q->where('professional_record_usage_register_id', $typeId);
            })
            ->delete();
    }
}
